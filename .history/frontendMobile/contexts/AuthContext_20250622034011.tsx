...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const logout = async () => {
    // No need to set loading, as we are clearing the session regardless
    try {
      await appwriteAuth.signOut();
    } catch (error) {
      // It's okay if this fails, it may mean the session is already expired.
      console.log('Ignoring sign-out error:', error);
    } finally {
